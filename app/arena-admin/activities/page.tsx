...
export const metadata = {
  title: "Activities Admin - KYL Arena",
  description: "KYL Arena Activities Operations Center",
};
...
export default async function ActivitiesAdminPage() {
  ...
  // Fetch all profiles to join against activities
  let dbProfiles: any[] = [];
  try {
    const { data, error: profError } = await supabaseAdmin
      .from("profiles")
      .select("id, name, avatar, email, strava_athlete_id");
    if (!profError && data) {
      dbProfiles = data;
    }
  } catch (e) {
