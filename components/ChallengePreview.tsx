"use client";

import { useEffect, useState } from "react";
import { Bike, Flame, Footprints, Medal } from "lucide-react";
import ScrollReveal from "@/components/ScrollReveal";
import { createClient } from "@/lib/supabase/client";
import { motion, AnimatePresence } from "framer-motion";

interface Participant {
  rank: number;
  user_id: string;
  name: string;
  avatar: string;
  value: number;
  percentage: number;
  completed: boolean;
}

interface ChallengeInfo {
  title: string;
  goal_target: number;
  goal_metric: string;
}

export default function ChallengePreview() {
  const [activeSport, setActiveSport] = useState("cycling");
  const [participants, setParticipants] = useState<Participant[]>([]);
  const [challengeInfo, setChallengeInfo] = useState<ChallengeInfo | null>(null);
  const [loading, setLoading] = useState(true);
  const supabase = createClient();

  useEffect(() => {
    let isMounted = true;
    
    const fetchLeaderboard = async () => {
      setLoading(true);
      const { data } = await supabase.rpc("get_live_leaderboard", {
        p_sport_type: activeSport,
      });
      
      if (!isMounted) return;

      if (data && data.length > 0) {
        setParticipants(data as any);
        setChallengeInfo({
          title: data[0].challenge_title,
          goal_target: data[0].challenge_goal_target,
          goal_metric: data[0].challenge_goal_metric,
        });
      } else {
        setParticipants([]);
        setChallengeInfo(null);
      }
      setLoading(false);
    };

    fetchLeaderboard();

    const channel = supabase
      .channel("public:leaderboard")
      .on("postgres_changes", { event: "*", schema: "public", table: "activities" }, fetchLeaderboard)
      .on("postgres_changes", { event: "*", schema: "public", table: "challenge_participants" }, fetchLeaderboard)
      .subscribe();

    return () => {
      isMounted = false;
      supabase.removeChannel(channel);
    };
  }, [activeSport, supabase]);

  const sports = [
    { id: "cycling", name: "Cycling", icon: <Bike className="h-5 w-5" /> },
    { id: "running", name: "Running", icon: <Flame className="h-5 w-5" /> },
    { id: "walking", name: "Walking", icon: <Footprints className="h-5 w-5" /> },
  ];

  const formatValue = (val: number, metric?: string) => {
    if (!metric) return `${val.toFixed(1)}`;
    if (metric === "Distance") return `${val.toFixed(1)} km`;
    if (metric === "Elevation") return `${val.toFixed(0)} m`;
    return `${val.toFixed(1)}`;
  };

  return (
    <section id="challenge-preview" className="py-20 bg-zinc-950/40 border-t border-white/5 scroll-mt-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        
        {/* Header */}
        <ScrollReveal className="mx-auto max-w-3xl text-center mb-12 space-y-4" direction="up">
          <h2 className="text-xs font-bold uppercase tracking-wider text-lime-400">
            Live Challenge Preview
          </h2>
          <p className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight">
            See the Arena in Action
          </p>
          <p className="text-base text-zinc-400">
            Toggle between sports below to view how automated leaderboards look, update, and inspire your community.
          </p>
        </ScrollReveal>

        {/* Interactive Sport Selectors */}
        <ScrollReveal className="flex justify-center gap-2 mb-8" direction="up" delayMs={100}>
          {sports.map((s) => (
            <button
              key={s.id}
              onClick={() => setActiveSport(s.id)}
              className={`flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold transition-all ${
                activeSport === s.id
                  ? "bg-lime-400 text-black shadow-lg shadow-lime-400/25"
                  : "bg-zinc-900 text-zinc-400 hover:text-white border border-white/5"
              }`}
            >
              {s.icon}
              {s.name}
            </button>
          ))}
        </ScrollReveal>

        {/* Leaderboard Card Mockup */}
        <ScrollReveal className="mx-auto max-w-2xl" direction="up" delayMs={200} zoom={true}>
          <div className="rounded-2xl border border-white/5 bg-zinc-900/20 p-5 sm:p-8 backdrop-blur-md shadow-2xl min-h-[400px]">

          {loading ? (
            <div className="flex items-center justify-center h-[300px]">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-lime-400" />
            </div>
          ) : challengeInfo ? (
            <>
              {/* Header Info */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-white/5 pb-5 mb-6 gap-3">
                <div>
                  <span className="text-[10px] uppercase font-mono tracking-widest text-lime-400 font-bold">
                    Active Challenge
                  </span>
                  <h3 className="text-xl font-bold text-white mt-0.5 truncate max-w-xs">{challengeInfo.title}</h3>
                </div>
                <div className="rounded-lg bg-zinc-800/50 px-4 py-2 border border-white/5 text-xs sm:text-right">
                  <span className="text-zinc-500 block">Goal Target</span>
                  <span className="font-semibold text-zinc-200">
                    {formatValue(challengeInfo.goal_target, challengeInfo.goal_metric)}
                  </span>
                </div>
              </div>

              {/* Leaders List */}
              <div className="space-y-4">
                <AnimatePresence>
                  {participants.map((p) => (
                    <motion.div
                      key={p.user_id}
                      layout
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className={`flex flex-col gap-2 rounded-xl p-4 transition-all duration-300 ${
                        p.completed
                          ? "bg-gradient-to-r from-lime-500/10 via-zinc-900/10 to-transparent border border-lime-500/10"
                          : "bg-zinc-900/30 border border-white/5"
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          {/* Rank Badge */}
                          <div className="flex items-center justify-center h-7 w-7 rounded-full bg-zinc-900 border border-white/10 shrink-0">
                            {p.rank === 1 ? (
                              <Medal className="h-4 w-4 text-amber-400" />
                            ) : p.rank === 2 ? (
                              <Medal className="h-4 w-4 text-zinc-350" />
                            ) : p.rank === 3 ? (
                              <Medal className="h-4 w-4 text-amber-600" />
                            ) : (
                              <span className="text-xs font-mono font-bold text-zinc-500">{p.rank}</span>
                            )}
                          </div>
                          
                          <div className="h-8 w-8 rounded-full overflow-hidden bg-zinc-800 border border-white/10 shrink-0 hidden sm:block">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={p.avatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&q=80"} alt={p.name} className="h-full w-full object-cover" />
                          </div>

                          <div className="min-w-0">
                            <span className="font-bold text-white text-sm sm:text-base truncate block max-w-[120px] sm:max-w-[200px]">
                              {p.name}
                            </span>
                            {p.completed && (
                              <span className="text-[9px] px-1.5 py-0.5 rounded bg-lime-400/10 text-lime-400 font-bold font-mono">
                                CLEARED!
                              </span>
                            )}
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="font-mono text-sm sm:text-base font-bold text-white">
                            {formatValue(p.value, challengeInfo.goal_metric)}
                          </span>
                          <span className="text-xs text-zinc-500 block">
                            {p.percentage.toFixed(0)}% completion
                          </span>
                        </div>
                      </div>

                      {/* Progress bar */}
                      <div className="w-full bg-zinc-950 h-2.5 rounded-full overflow-hidden border border-white/5 mt-1 relative">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${p.percentage}%` }}
                          transition={{ duration: 1, ease: "easeOut" }}
                          className={`absolute left-0 top-0 h-full rounded-full ${
                            p.completed ? "bg-lime-400" : "bg-zinc-700"
                          }`}
                        />
                      </div>
                    </motion.div>
                  ))}
                </AnimatePresence>
                
                {participants.length === 0 && (
                  <div className="text-center py-8 text-zinc-500 text-sm">
                    No participants have joined yet.
                  </div>
                )}
              </div>

              {/* Empty state / footer info */}
              <div className="mt-6 flex items-center justify-between text-xs text-zinc-500 border-t border-white/5 pt-4">
                <span className="flex items-center gap-1.5">
                  <span className="relative flex h-2 w-2">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-lime-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2 w-2 bg-lime-500"></span>
                  </span>
                  Live Updates
                </span>
                <span>Strava API Sync v1.0</span>
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center justify-center h-[300px] text-center text-zinc-500 space-y-3">
              <Medal className="h-10 w-10 text-zinc-700" />
              <p>No active {activeSport} challenges right now.</p>
            </div>
          )}

        </div>
        </ScrollReveal>

      </div>
    </section>
  );
}
